...
extension CategoriesCollectionView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        categoryDetailViewController.configureCategoryDetailViewController(
            with: indexPath.item,
            categoryDetailTitle: inputCategories?.getTitle(at: indexPath.item) ?? "",
            linkedWordsId: inputCategories?.getLinkedWordsId(at: indexPath.item) ?? "",
            updateCountWordsDelegate: self.updateCountWordsDelegate)
